...
class Organism():
    def __init__(self, generation, c, parent, w, h, current_poly):
    ...
    def random_mutation(self, number):
        ...
        options = [self.move_vertex, self.change_color]

        for i in range(0, number):
            mutation = choice(options)
            mutation()

    def recomposed_random_mutation(self, number):
        # performs number random mutations
        # note: function names of all available mutations have to be hardcoded in list 'options' below
        options = [self.recomposed_gene_jump, self.recomposed_move_vertex, self.recomposed_transfer_vertex, self.recomposed_change_color]
    ...
    def recomposed_move_vertex(self):
        # change a (x, y) coord of a vertex of a random generation
        xy = (randint(0,self.w), randint(0,self.h))
        i = randint(0, len(self.genome) - 1)
        v = randint(0, len(self.genome[i][0]) - 1)
    ...
    def recomposed_transfer_vertex(self):
    ...
    def recomposed_change_color(self):
        # changes one channel of the color (or the alpha) of a random polygon
        i = randint(0, len(self.genome) - 1)
        j = randint(0, 4)
    ...
    def save_img(self, directory):
        filename = directory + "/" + "{:0>3}".format(self.current_poly) + "-" + "{:0>6}".format(self.generation) + "-" + "{:0>3}".format(self.id) + "-" + str(self.fitness) + ".png"
        im = Image.fromarray(self.array)
        im.save(filename)

    def save_polygons(self, directory):
        filename = directory + "/" + "{:0>3}".format(self.current_poly) + "-" + "{:0>6}".format(self.generation) + "-" + "{:0>3}".format(self.id) + "-" + str(self.fitness) + ".txt"
