...
class Organism():
    def __init__(self, generation, c, parent, w, h, current_batch, num_batch, num_poly):
    ...
    def initialize_genome(self, genomesize, numvertex):
        ...
        for i in range(0, genomesize):
            ...
            color = (randint(0,255),randint(0,255),randint(0,255), 0)
    ...
    def random_mutation(self, number):
        ...
        options = [self.move_vertex, self.change_color, self.transfer_vertex, self.gene_jump]
    ...
    def gene_jump(self):
        ...
        i = randint(int((self.num_poly/self.num_batch) * self.current_batch), int((self.num_poly/self.num_batch) + ((self.num_poly/self.num_batch) * self.current_batch)) - 1)
        j = randint(int((self.num_poly/self.num_batch) * self.current_batch), int((self.num_poly/self.num_batch) + ((self.num_poly/self.num_batch) * self.current_batch)) - 1)
    ...
    def transfer_vertex(self):
        ...
        giver = randint(int((self.num_poly/self.num_batch) * self.current_batch), int((self.num_poly/self.num_batch) + ((self.num_poly/self.num_batch) * self.current_batch)) - 1)
        receiver = randint(int((self.num_poly/self.num_batch) * self.current_batch), int((self.num_poly/self.num_batch) + ((self.num_poly/self.num_batch) * self.current_batch)) - 1)
        ...
        while True:
            ...
            giver = randint(int((self.num_poly/self.num_batch) * self.current_batch), int((self.num_poly/self.num_batch) + ((self.num_poly/self.num_batch) * self.current_batch)) - 1)
            receiver = randint(int((self.num_poly/self.num_batch) * self.current_batch), int((self.num_poly/self.num_batch) + ((self.num_poly/self.num_batch) * self.current_batch)) - 1)
            if (giver != receiver and len(self.genome[giver][0]) > 3) or count > 100:
                break
    ...
    def set_alpha_value(self):
        for i in range(int((self.num_poly/self.num_batch) * self.current_batch), int((self.num_poly/self.num_batch) + ((self.num_poly/self.num_batch) * self.current_batch)) - 1):
            color = self.genome[i][1][0], self.genome[i][1][1], self.genome[i][1][2], randint(0,255)
            new_gene = (self.genome[i][0], color)
            self.genome[i] = new_gene

    def save_img(self, directory):
        filename = directory + "/" + "{:0>3}".format(self.current_batch) + "-" + "{:0>6}".format(self.generation) + "-" + "{:0>3}".format(self.id) + "-" + str(self.fitness) + ".png"
        im = Image.fromarray(self.array)
        im.save(filename)

    def save_polygons(self, directory):
        filename = directory + "/" + "{:0>3}".format(self.current_batch) + "-" + "{:0>6}".format(self.generation) + "-" + "{:0>3}".format(self.id) + "-" + str(self.fitness) + ".txt"
